...
namespace Gtk.DataBindings
{
	...
	public class DataIconView : IconView, IAdaptableListControl, IPostableControl, IAdaptableControl
	{
...
//			InsertValuesFromObjectToIter (obj, iter);
		...
		/// </summary>
		/// <param name="iter">
		/// TreeIter where ActiveIter is returned <see cref="TreeIter"/>
		/// </param>
		/// <returns>
		/// Returns true if successful <see cref="System.Boolean"/>
		/// </returns>
		private bool GetActiveIter (out TreeIter iter)
		{
			iter = new TreeIter();
			TreePath tp;
			CellRenderer cr;
			if (GetCursor (out tp, out cr) == false)
				return (false);
			return (Model.GetIter (out iter, tp));
		}
...
//			cachedItems = null;
		...
		/// Mappings with this widget <see cref="System.String"/>
		/// </param>
		public DataIconView (object aDataSource, string aMappings)
			: base()
		{
			internalModel = new MappingsImplementor (this);
			CreateAdaptors();
