// DataCalendar.cs - DataCalendar implementation for Gtk#Databindings
//
...
namespace Gtk.DataBindings
{
	...
	public class DataCalendar : Calendar, IAdaptableControl, IPostableControl
	{
		...
		public virtual void PutDataToDataSource (object aSender)
		{
			...
			if ((DateTime) adaptor.Value != Date)
				adaptor.Value = Date;
		}
		...
		{
			// Important!!!
			// Set adaptor.DataChanged to false, control will load its value trough
			// adaptor, which means values should be equal
			adaptor.DataChanged = false;
			// Simply load main value, all other mapped values will be resolved
			// by adaptor engine to minimize the possibillity of forgetting to
			// do this step in any widget subclass that supports multiple mappings 
			Date = (DateTime) Adaptor.Value;
		}
